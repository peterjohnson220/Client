...
export class MockActivityEndpoint {
  static getTimelineActivities(typeFilter: string[]): Observable<TimelineActivityDto[]> {
    const timelineActivities = [];

    if (typeFilter.indexOf('CommunityPost') !== -1 ) {
    ...
    if (typeFilter.indexOf('ResourcesPost') !== -1) {
       timelineActivities.push({
        Id: 1,
        Type: 'ResourcesPost',
        SubType: null,
    ...
    if (typeFilter.indexOf('ActivityPost') !== -1) {
      timelineActivities.push({
        Id: 1,
        Type: 'ActivityPost',
        SubType: null,
