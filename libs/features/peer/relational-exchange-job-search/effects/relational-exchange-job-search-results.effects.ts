...
import { RelationalExchangeJobSearchResponse } from 'libs/models/payfactors-api/exchange-job-search';
...
@Injectable()
export class RelationalExchangeJobSearchResultsEffects {
    ...
    return action$
      .pipe(
        ...
        switchMap((data) => {
          ...
          return this.exchangeJobSearchApiService.getRelationalExchangeJobSearchResults(searchRequest).pipe(
            mergeMap((response: RelationalExchangeJobSearchResponse) => {
              const actions = [];
              ...
              const exchangeJobResults = !!response?.ExchangeJobResults?.length ? response.ExchangeJobResults: [];
