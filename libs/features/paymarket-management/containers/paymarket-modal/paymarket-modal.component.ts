...
export class PayMarketModalComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.payMarketIdSubscription = this.payMarketId$.subscribe(payMarketId => this.payMarketId = payMarketId);
  ...
  handleTabChange(event: NgbTabChangeEvent): void {
    if (event.nextId === this.payMarketModalTabs.DefaultScopes && !!this.payMarketId) {
      this.store.dispatch(new fromDefaultScopesActions.LoadDefaultScopes({ payMarketId: this.payMarketId }));
    } else if (event.nextId === this.payMarketModalTabs.ExchangeScopes && !!this.payMarketId) {
