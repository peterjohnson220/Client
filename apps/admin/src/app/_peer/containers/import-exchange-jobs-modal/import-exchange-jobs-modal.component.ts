...
export class ImportExchangeJobsModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.store.select(fromPeerAdminReducer.getStoredDataFile).subscribe(sd => this.storedDataFile = sd);
    this.store.select(fromPeerAdminReducer.getIsFileValid).subscribe( isValidImport => {
      if (isValidImport) {
