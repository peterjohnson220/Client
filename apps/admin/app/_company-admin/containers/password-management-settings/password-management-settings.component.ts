...
export class PasswordManagementSettingsComponent implements OnInit, OnDestroy {
  ...
  constructor(private store: Store<fromCompanyAdminReducer.State>,
              private rootStore: Store<fromRootReducer.State>,
              private formBuilder: FormBuilder) {}

  ngOnInit() {
    this.store.dispatch(new fromCompanyAdminActions.LoadCompanyAdminPasswordSettings());
    this.passwordSettingsLoading$ = this.store.select(fromCompanyAdminReducer.getPasswordSettingsLoading);
    this.passwordSettingsLoadingError$ = this.store.select(fromCompanyAdminReducer.getPasswordSettingsLoadingError);
    this.passwordSettings$ = this.store.select(fromCompanyAdminReducer.getPasswordSettings);
