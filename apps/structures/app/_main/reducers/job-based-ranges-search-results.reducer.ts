...
export function reducer(state = initialState, action: fromJobBasedRangesSearchResultsActions.Actions): State {
  switch (action.type) {
    case fromJobBasedRangesSearchResultsActions.TOGGLE_JOB_SELECTION: {
    ...
    case fromJobBasedRangesSearchResultsActions.LOAD_JOB_PRICING_DATA: {
      const jobsCopy = cloneDeep(state.jobs);
    ...
    case fromJobBasedRangesSearchResultsActions.LOAD_JOB_PRICING_DATA_SUCCESS: {
      const jobsCopy = cloneDeep(state.jobs);
    ...
    case fromJobBasedRangesSearchResultsActions.LOAD_JOB_PRICING_DATA_ERROR: {
      const jobsCopy = cloneDeep(state.jobs);
    ...
    case fromJobBasedRangesSearchResultsActions.TOGGLE_JOB_DETAIL: {
      const jobsCopy = cloneDeep(state.jobs);
      const jobToUpdate = jobsCopy.find(x => x.Id === action.payload.Id);
      if (jobToUpdate) {
