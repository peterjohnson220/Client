...
import * as fromJobBasedRangesAddJobsSearchResultsActions from '../actions/job-based-ranges-search-results.actions';
import * as fromStructuresReducer from '../reducers';

@Injectable()
export class JobBasedRangesSearchResultsEffects {
  ...
  loadPricingData$ = this.actions$
    .pipe(
      ofType(fromJobBasedRangesAddJobsSearchResultsActions.LOAD_JOB_PRICING_DATA),
      withLatestFrom(
        this.store.select(fromStructuresReducer.getContext),
        (action: fromJobBasedRangesAddJobsSearchResultsActions.GetJobPricingData, context) => (
          { action, context }
        )),
      mergeMap((data) => {
        let jobCode = null;
        ...
        return this.jobSearchApiService.getJobPricingData({
          ...
          .pipe(
            map((pricingDataResponse: JobSearchPricingDataResponse) =>
              new fromJobBasedRangesAddJobsSearchResultsActions.GetJobPricingDataSuccess(
                {
                  jobId: jobResult.Id,
            ...
            catchError(() => of(new fromJobBasedRangesAddJobsSearchResultsActions.GetJobPricingDataError(jobResult.Id)))
          );
      }
      ));
          ...
          .pipe(
            mergeMap((searchResponse: JobSearchResponse) => {
              ...
              if (searchRequest.PagingOptions.From > 0) {
                ...
                actions.push(new fromJobBasedRangesAddJobsSearchResultsActions.AddJobResults(
                ...
                actions.push(new fromJobBasedRangesAddJobsSearchResultsActions.ReplaceJobResults(
                  PayfactorsAddJobsApiModelMapper.mapJobSearchResultsToJobResults(searchResponse.JobResults)
                ));
