...
export class HistoricalTrendChartComponent implements OnInit, OnDestroy, OnChanges {
  ...
  constructor(private store: Store<fromComphubPeerTrendsDataReducers.State>,
    ...
    this.selectedPeerTrendId$ = this.store.select(fromComphubPeerTrendsDataReducers.getSelectedTrendId);
    this.trendChartDomain$ = this.store.select(fromComphubPeerTrendsDataReducers.getPeerTrendsDomain);
  }
