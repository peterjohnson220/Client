...
export class QuickPriceHistoryComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(
    ...
    this.loadingJobDataHistory$ = this.store.select(fromComphubSharedReducer.getLoadingJobDataHistory);
    this.loadingJobDataErrorMessage$ = this.store.select(fromComphubSharedReducer.getLoadingJobDataHistoryErrorMessage);
  }
