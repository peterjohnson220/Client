...
import { Filter, FilterType, MultiSelectOption, ProjectSearchContext } from '../../models';
...
export class SearchFiltersComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.filters$ = this.store.select(fromSharedSearchReducer.getFilters);
    this.pageShown$ = this.store.select(fromSharedSearchReducer.getPageShown);
    this.projectSearchContext$ = this.store.select(fromSharedSearchReducer.getProjectSearchContext);
    this.filterSearchVisible$ = this.store.select(fromSharedSearchReducer.getSearchingFilter);
  }
